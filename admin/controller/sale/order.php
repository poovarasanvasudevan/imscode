...
class ControllerSaleOrder extends Controller
{
    ...
    public function update()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_sale_order->addInvoice($this->request->get['order_id'], $this->request->post);
            ...
            $shp_quantity1 = 0;
            $shp_quantity2 = 0;
            $order_id1 = $this->request->get['order_id'];
            foreach ($this->request->post['order_invoice'] as $order_invoice) {
                $shp_quantity1 = (int)$order_invoice['inv_quantity'] + $this->model_sale_order->getShippedQuantity($order_id1, $order_invoice['order_product_id']);
                $shp_quantity2 += $shp_quantity1;
                if ($shp_quantity1 > 0) {
                    $this->model_sale_order->updateShipQuantity($order_id1, $order_invoice, $shp_quantity1, $order_status_id);
                }
            }

            $this->model_sale_order->updateOrderStatus($order_id1, $order_invoice, $shp_quantity2, $order_status_id);
    ...
    public function complete()
    {
        ...
        if (isset($this->request->post['selected']) && ($this->validateDelete())) {
            foreach ($this->request->post['selected'] as $order_id) {
                $this->model_sale_order->completeOrder($order_id);
            }
    ...
    public function updateorder()
    {
        ...
        if (isset($this->request->post['selected']) && ($this->validateDelete()) && isset($this->request->post['filter_order_status_id'])) {
            foreach ($this->request->post['selected'] as $order_id) {
                $this->model_sale_order->updateOrderStatusDup($order_id, $this->request->post['filter_order_status_id']);
            }
    ...
    protected function getList()
    {
        ...
        $this->data['complete'] = $this->url->link('sale/order/complete', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $this->data['update'] = $this->url->link('sale/order/updateorder', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        $this->data['button_complete'] = $this->language->get('button_complete');
    ...
    public function getForm()
    {
        ...
        if (isset($this->error['inv_quantity'])) {
            $this->data['error_inv_quantity'] = $this->error['inv_quantity'];
        } else {
            $this->data['error_inv_quantity'] = '';
        }
        ...
        /*if (isset($this->request->post['shp_quantity'])) {
         $this->data['shp_quantity'] = $this->request->post['shp_quantity'];
        } elseif (!empty($order_info)) {
        $this->data['shp_quantity'] = $order_info['shp_quantity'];
        } else {
        $this->data['shp_quantity'] = '';
        }*/
        ...
        foreach ($order_products as $order_product) {
            ...
            $this->data['order_products'][] = array(
                ...
                'inv_total' => $order_product['shp_quantity'] * $order_product['price'],
        ...
        foreach ($order_invoices as $order_invoice) {
            ...
            if (($order_invoice['quantity'] == $order_invoice['shp_quantity']) || $order_status_id == 3) {
    ...
    protected function validateForm()
    {
        ...
        //get already shipped quantity
        $order_id1 = $this->request->get['order_id'];

        $valid_quantity = true;
        foreach ($this->request->post['order_invoice'] as $order_invoice) {
            $shp_quantity = $this->model_sale_order->getShippedQuantity($order_id1, $order_invoice['order_product_id']);
            if (!empty($order_invoice['inv_quantity'])) {
                $inv_quantity1 = (int)$order_invoice['inv_quantity'];
            } else {
                $inv_quantity1 = 0;
            }
            if (($inv_quantity1 < 0) || (($shp_quantity + $inv_quantity1) > (int)$order_invoice['quantity'])) {
                $this->error['inv_quantity'] = $this->language->get('error_inv_quantity');
                break;
            }

            /*$inv_quantity1 = (int)$order_invoice['inv_quantity'];
            $quantity1 = (int)$order_invoice['quantity'];

            //if (((int)$inv_quantity1 > (int)$quantity1) || ((int)$inv_quantity1 > (int)$shp_quantity)) {
            /*if(1 > 3 ){
        ...
        if ($valid_quantity == false) {
            $this->error['error_inv_quantity'] = $this->language->get('error_postcode');
        }
    ...
    public function info()
    {
        ...
        if ($order_info) {
            ...
            $this->data['order_weight'] = $order_info['order_weight'];
            ...
            foreach ($products as $product) {
                ...
                $this->data['products'][] = array(
                    ...
                    'inv_quantity' => $order_info['inv_quantity'],
    ...
    public function notifyDispatcher($order_id, $order_status_id, $comment)
    {
        $order_info = $this->model_sale_order->getOrder($order_id);
        ...
        if ($order_status_query->num_rows) {
            $message .= sprintf($language->get('text_notify_dispatcher'), $order_id) . "\n";
        ...
        $mail->setFrom($this->config->get('config_dispatcher_email'));
    ...
    function history()
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            } //Do not allow to change to Partial Shippment from this window. Instead partial shipment can ONLy be done from Sale->Order->Edit window
            elseif ($this->request->post['order_status_id'] == 18) {
                $this->data['error'] = $this->language->get('error_partial_invoice');
            }

            if (!$this->data['error']) {
                $this->model_sale_order->addOrderHistory($this->request->get['order_id'], $this->request->post);

                if ($this->request->post['order_status_id'] == 2) {
                    //$this->model_sale_order->updateShipQuantity($this->request->get['order_id'], $order_invoice, $shp_quantity1, $this->request->post['order_status_id']);
                    $this->createInvoiceNo();
                    $this->notifyDispatcher($this->request->get['order_id'], $this->request->post['order_status_id'], $this->request->post['comment']);
                }
            ...
            if ($order_info) {
                ...
                foreach ($products as $product) {
                    ...
                    $product_data[] = array(
                        ...
                        'price' => $this->currency->format(($product['price'] - $product['tax']) + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
                        'total' => $this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value'])
                    );
                }
                ...
                $this->data['orders'][] = array(
                    ...
                    'invoice_date' => date($this->language->get('date_format_short'), strtotime(date("Y-m-d"))),
