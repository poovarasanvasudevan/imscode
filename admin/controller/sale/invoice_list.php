<?php
class ControllerSaleInvoiceList extends Controller {
	private $error = array();

  	public function index() {
		$this->load->language('sale/invoice_list');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('sale/invoice_list');
  	...
  	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_invoice_list->editInvoice($this->request->get['order_id'], $this->request->post);
			...
			if (isset($this->request->get['filter_inv_quantity'])) {
				$url .= '&filter_inv_quantity=' . $this->request->get['filter_inv_quantity'];
			}
			...
			$this->redirect($this->url->link('sale/invoice_list', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
  	...
  	public function delete() {
    	...
    	if (isset($this->request->post['selected']) && ($this->validateDelete())) {
			foreach ($this->request->post['selected'] as $invoice_no) {
				$this->model_sale_invoice_list->updateShipQuantity($invoice_no);
				$this->model_sale_invoice_list->deleteInvoice($invoice_no);
			}
  	...
  	private function getList() {
		...
		if (isset($this->request->get['filter_inv_quantity'])) {
			$filter_inv_quantity = $this->request->get['filter_inv_quantity'];
		} else {
			$filter_inv_quantity = null;
		}
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('sale/invoice_list', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);

		$this->data['invoice'] = $this->url->link('sale/invoice_list/invoice', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('sale/invoice_list/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$data = array(
			...
			'filter_inv_quantity'	 	 => $filter_inv_quantity,
		...
		$order_total = $this->model_sale_invoice_list->getTotalOrders($data);

		$invoices = $this->model_sale_invoice_list->getInvoices($data);
    	...
    	foreach ($invoices as $invoice) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('sale/invoice_list/info', 'token=' . $this->session->data['token'] . '&order_id=' . $invoice['order_id'] . $url, 'SSL')
			);
			
			$currency_code = "INR"; //to be changed to read from the configuration
			$currency_value = 1;  //to be changed to read from the configuration
			
			//get tax rate for the product id
			$order_tax_rate = $this->model_sale_invoice_list->getOrderProductTaxRate($invoice['order_product_id']);
			
			$this->data['invoices'][] = array(
				'order_id'      => $invoice['order_id'],
				'order_product_id'  => $invoice['order_product_id'],
				'invoice_id'    => $invoice['invoice_id'],
				'invoice_no'    => $invoice['invoice_no'],
				'inv_quantity'  => $invoice['inv_quantity'],
				'tax'			=> $order_tax_rate,
				'price'         => $invoice['price'],
				'total'         => $this->currency->format($invoice['total'], $currency_code, $currency_value),
				'selected'      => isset($this->request->post['selected']) && in_array($invoice['order_id'], $this->request->post['selected']),
				'action'        => $action,
				'date'			=> $invoice['date_added'] == null ? "--" : $invoice['date_added'],
				'order_status'  => $invoice['name'] == null ? "--" : $invoice['name']
			);
		}
    	...
    	$this->data['column_shp_quantity'] = $this->language->get('column_shp_quantity');
    	$this->data['column_inv_quantity'] = $this->language->get('column_inv_quantity');
		...
		if (isset($this->request->get['invoice_list'])) {
			$url .= '&order=' . $this->request->get['invoice_list'];
		}
		...
		$pagination->url = $this->url->link('sale/invoice_list', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->data['filter_inv_quantity'] = $filter_inv_quantity;
  	...
  	public function getForm() {
		...
		$this->data['button_gen_invoice'] = $this->language->get('button_gen_invoice');
		...
		if (isset($this->error['payment_id'])) {
			$this->data['error_payment_id'] = $this->error['payment_id'];
		} else {
			$this->data['error_payment_id'] = '';
		}
		
		if (isset($this->error['payment_tran_id'])) {
			$this->data['error_payment_tran_id'] = $this->error['payment_tran_id'];
		} else {
			$this->data['error_payment_tran_id'] = '';
		}
		
		if (isset($this->error['payment_ref_id'])) {
			$this->data['error_payment_ref_id'] = $this->error['payment_ref_id'];
		} else {
			$this->data['error_payment_ref_id'] = '';
		}
    	...
    	if (isset($this->request->post['payment_tran_id'])) {
    		$this->data['payment_tran_id'] = $this->request->post['payment_tran_id'];
    	} elseif (!empty($order_info)) {
    		$this->data['payment_tran_id'] = $order_info['payment_tran_id'];
    	} else {
    		$this->data['payment_tran_id'] = '';
    	}
    	
    	if (isset($this->request->post['payment_ref_id'])) {
    		$this->data['payment_ref_id'] = $this->request->post['payment_ref_id'];
    	} elseif (!empty($order_info)) {
    		$this->data['payment_ref_id'] = $order_info['payment_ref_id'];
    	} else {
    		$this->data['payment_ref_id'] = '';
    	}
		...
		if (isset($this->request->post['order_invoice'])) {
			$order_invoices = $this->request->post['order_invoice'];
		} elseif (isset($this->request->get['order_id'])) {
			$order_invoices = $this->model_sale_order->getOrderProducts($this->request->get['order_id']);
		} else {
		...
		foreach ($order_products as $order_product) {
			...
			$this->data['order_products'][] = array(
				...
				'shp_quantity'     => $order_product['shp_quantity'],
				'inv_quantity'     => $order_product['inv_quantity'],
				'price'            => $order_product['price'],
				'total'            => $order_product['total'],
				'shp_total'		   => $order_product['shp_quantity'] * $order_product['price'],
				'inv_total'		   => $order_product['inv_quantity'] * $order_product['price'],
				'tax'              => $order_product['tax'],
		...
		//order invoices
		$this->data['order_invoices'] = array();
		
		$inv_prev_no = '';
		$row_span = 1;
		foreach ($order_invoices as $order_invoice) {
			if (isset($order_invoice['order_option'])) {
				$order_option = $order_invoice['order_option'];
			} elseif (isset($this->request->get['order_id'])) {
				$order_option = $this->model_sale_order->getOrderOptions($this->request->get['order_id'], $order_invoice['order_product_id']);
			} else {
				$order_option = array();
			}
		
			if (isset($order_invoice['order_download'])) {
				$order_download = $order_invoice['order_download'];
			} elseif (isset($this->request->get['order_id'])) {
				$order_download = $this->model_sale_order->getOrderDownloads($this->request->get['order_id'], $order_invoice['order_product_id']);
			} else {
				$order_download = array();
			}
			...
			if ($inv_prev_no == $invoice_no){
				$row_span++;
			}
			else{
				$inv_prev_no = $invoice_no;
			}
				
			$this->data['order_invoices'][] = array(
					'order_product_id' => $order_invoice['order_product_id'],
					'product_id'       => $order_invoice['product_id'],
					'invoice_no'	   => $invoice_no,
					'name'             => $order_invoice['name'],
					'model'            => $order_invoice['model'],
					'option'           => $order_option,
					'download'         => $order_download,
					'quantity'         => $order_invoice['quantity'],
					'shp_quantity'     => $order_invoice['shp_quantity'],
					'inv_quantity'     => $order_invoice['inv_quantity'],
					'price'            => $order_invoice['price'],
					'total'            => $order_invoice['total'],
					'shp_total'		   => $order_invoice['shp_quantity'] * $order_invoice['price'],
					'inv_total'		   => $order_invoice['inv_quantity'] * $order_invoice['price'],
					'tax'              => $order_invoice['tax'],
					'reward'           => $order_invoice['reward']
			);
		}
		
		$this->data['row_span'] = $row_span;
  	...
  	private function validateForm() {
    	...
    	foreach ($this->request->post['order_product'] as $order_product) {
    		$shp_quantity1 = (int)$order_product['shp_quantity'];
    		$quantity1 = (int)$order_product['quantity'];
    		if ($shp_quantity1 > $quantity1) {
    			$valid_quantity = false;
    		}
    	}
    	
    	if($valid_quanity == false){
    	}
		...
		/*Commenting to disable Vouchers *
		 * if (!$this->request->post['payment_id']) {
			$this->error['payment_id'] = $this->language->get('error_payment');
		}
		
		if (!$this->request->post['payment_tran_id']) {
			$this->error['payment_tran_id'] = $this->language->get('error_payment');
		}
		
		if (!$this->request->post['payment_ref_id']) {
			$this->error['payment_ref_id'] = $this->language->get('error_payment');
		}*/
		
		/*if (!$this->request->post['shp_quantity']) {
			$this->error['shp_quantity'] = $this->language->get('error_shp_quantity');
		}*/
	...
	public function info() {
		...
		if ($order_info) {
			...
			$this->data['text_payment_id'] = $this->language->get('text_payment_id');
			$this->data['text_payment_tran_id'] = $this->language->get('text_payment_tran_id');
			$this->data['text_payment_ref_id'] = $this->language->get('text_payment_ref_id');
			...
			$this->data['entry_order_status'] = $this->language->get('entry_order_status');
			$this->data['entry_shp_quantity'] = $this->language->get('entry_shp_quantity');
			...
			$this->data['tab_order_invoice'] = $this->language->get('tab_order_invoice');
			...
			$this->data['invoice'] = $this->url->link('sale/invoice_list/invoice', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$this->data['cancel'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . $url, 'SSL');
			...
			//$this->data['shp_quantity'] = nl2br($order_info['shp_quantity']);
			$this->data['shipping_method'] = $order_info['shipping_method'];
			$this->data['payment_method'] = $order_info['payment_method'];
			$this->data['payment_id'] = $order_info['payment_id'];
			$this->data['payment_tran_id'] = $order_info['payment_tran_id'];
			...
			foreach ($products as $product) {
				...
				$this->data['products'][] = array(
					...
					'shp_quantity'	   => $product['shp_quantity'],
					'inv_quantity'	   => $product['inv_quantity'],
					'price'    		   => $this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
					'total'    		   => $this->currency->format(($product['price'] * $product['quantity']) + ($this->config->get('config_tax') ? ($product['tax'] * $product['shp_quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']),
	...
	public function history() {
		...
		foreach ($results as $result) {
        	$this->data['histories'][] = array(
        		...
        		'shp_quantity'    => nl2br($result['shp_quantity']),
	...
	public function upload() {
  	...
  	public function invoice() {
		...
		$this->data['text_payment_ref_id'] = $this->language->get('text_payment_ref_id');
		$this->data['text_shipping_method'] = $this->language->get('text_shipping_method');
		$this->data['text_admin_address'] = $this->language->get('text_admin_address');
		...
		} elseif (isset($this->request->get['order_product_id'])) {
			$orders[] = $this->request->get['order_product_id'];
		}

		foreach ($orders as $invoice_no) {
			$order_id = $this->model_sale_invoice_list->getOrderByInvoiceNo($invoice_no);
			
			$order_info = $this->model_sale_invoice_list->getInvoice($order_id);
			...
			if ($order_info) {
				...
				if ($store_info) {
					...
					$store_tin = $store_info['config_tin'];
					$store_cst = $store_info['config_cst'];
					$store_telephone = $store_info['config_telephone'];
					$store_fax = $store_info['config_fax'];
				} else {
					$store_address = $this->config->get('config_address');
					$store_email = $this->config->get('config_email');
					$store_tin = $this->config->get('config_tin');
					$store_cst = $this->config->get('config_cst');
				...
				$products = $this->model_sale_invoice_list->getInvoiceProducts($invoice_no);
				
				$total_quantity = $this->model_sale_invoice_list->getTotalQuantityByOrderId($order_id);
				...
				foreach ($products as $product) {
					...
					$options = $this->model_sale_invoice_list->getOrderOptions($order_id, $product['order_product_id']);
					
					$orders = $this->model_sale_invoice_list->getOrderProductsByProductId($product['order_product_id']);
					
					$products_tax = $this->model_sale_invoice_list->getOrderProductsByProductId($product['order_product_id']);
					...
					$sub_total = $sub_total + ($product['price'] * $product['inv_quantity']) + ($this->config->get('config_tax') ? ($products_tax[0]['tax'] * $product['inv_quantity']) : 0);
					
					$total_inv_qty = $total_inv_qty + $product['inv_quantity'];
					
					$product_data[] = array(
						'name'     => $orders[0]['name'],
						'model'    => $orders[0]['model'],
						'option'   => $option_data,
						'quantity' => $product['inv_quantity'],
						'tax'	   => $this->currency->format($products_tax[0]['tax']),
						'price'    => $this->currency->format(($product['price'] - $products_tax[0]['tax']) + ($this->config->get('config_tax') ? $products_tax[0]['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
						'total'    => $this->currency->format(($product['price'] * $product['inv_quantity']) + ($this->config->get('config_tax') ? ($products_tax[0]['tax'] * $product['inv_quantity']) : 0), $order_info['currency_code'], $order_info['currency_value'])
					);
				}
				
				$voucher_data = array();
				
				$vouchers = $this->model_sale_invoice_list->getOrderVouchers($order_id);
				...
				$total_data = $this->model_sale_invoice_list->getOrderTotals($order_id);
				
				foreach ($total_data as &$total){
					if(strcmp($total['title'], 'Sub-Total') == 0){
						$total['text'] = $this->currency->format($sub_total, $order_info['currency_code'], $order_info['currency_value']);
					}
					elseif(strcmp($total['title'], 'Total') != 0){
						if(strcmp($total['title'], 'Shipping') == 0){
							$total['value'] = $total['value'] * $total_inv_qty/$total_quantity;
							$total['text'] = "$" . number_format($total['value'], 2);
							$total1 = $sub_total + $total['value'];
						}
						else{
							$total1 = $sub_total + $total['value'];
						}
					...
					if(strcmp($total['title'], 'Total') == 0){
						$total['text'] = $this->currency->format($total1, $order_info['currency_code'], $order_info['currency_value']);
					}
				}
				...
				$this->data['orders'][] = array(
					...
					'invoice_date'       => date($this->language->get('date_format_short'), strtotime($order_info['date_added'])),
					...
					'payment_tran_id'    => $order_info['payment_tran_id'],
					'payment_ref_id'     => $order_info['payment_ref_id'],
