...
class ModelSaleInvoiceList extends Model
{
    /*public function addOrder($data) {

        echo '<script type="text/javascript">alert("Inside addOrder function of order.php");</script>';
    ...
    public function editInvoice($order_id, $data)
    {
        $this->load->model('localisation/country');
        ...
        if (isset($data['order_product'])) {
            foreach ($data['order_product'] as $order_product) {
                ...
                $msg = $order_product['shp_quantity'];
                $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_product_id = '" . (int)$order_product['order_product_id'] . "', order_id = '" . (int)$order_id . "', product_id = '" . (int)$order_product['product_id'] . "', name = '" . $this->db->escape($order_product['name']) . "', model = '" . $this->db->escape($order_product['model']) . "', quantity = '" . (int)$order_product['quantity'] . "', shp_quantity = '" . (int)$order_product['shp_quantity'] . "', inv_quantity = '" . (int)$order_product['inv_quantity'] . "', price = '" . (float)$order_product['price'] . "', total = '" . (float)$order_product['total'] . "', tax = '" . (float)$order_product['tax'] . "', reward = '" . (int)$order_product['reward'] . "'");
    ...
    {
        $invoice_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "invoice` WHERE invoice_no = '" . $invoice_no . "'");

        if ($invoice_query->num_rows) {
            foreach ($invoice_query->rows as $invoice) {
                //$this->log->write('\r\n from model --->>>  invoice_no =  ' . $invoice_no);
                $this->db->query("DELETE FROM `" . DB_PREFIX . "invoice` WHERE invoice_no = '" . $invoice_no . "'");

                //$this->db->query("UPDATE `" . DB_PREFIX . "order_product` SET shp_quantity = (shp_quantity - " . (int)$invoice['inv_quantity'] . ") WHERE order_product_id = '" . (int)$invoice['order_product_id'] . "' AND order_id = '" . (int)$invoice['order_id'] . "'");
            }
        }
    }

    //Prabhu
    public function updateShipQuantity($invoice_no)
    {
        $invoice_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "invoice` WHERE invoice_no = '" . $invoice_no . "'");
        ...
        if ($invoice_query->num_rows) {
            foreach ($invoice_query->rows as $invoice) {
                $this->db->query("UPDATE `" . DB_PREFIX . "order_product` SET shp_quantity = (shp_quantity - " . (int)$invoice['inv_quantity'] . ") WHERE order_product_id = '" . (int)$invoice['order_product_id'] . "' AND order_id = '" . (int)$invoice['order_id'] . "'");
                $order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_product` WHERE order_product_id = '" . $invoice['order_product_id'] . "'");
                foreach ($order_query->rows as $order) {
                    if (($order['shp_quantity'] == 0)) {
                        $order_status_id = 15;
                        $query = "UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . $order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$invoice['order_id'] . "'";
                        $this->db->query($query);
                    } elseif ((($order['shp_quantity']) == $order['quantity'])) {
                        $order_status_id = 3;
                        $query = "UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . $order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$invoice['order_id'] . "'";
                        $this->db->query($query);
                    } elseif ((($invoice['shp_quantity'] + $order['shp_quantity']) < $order['quantity']) && ($invoice['inv_quantity'] > 0)) {
                        $order_status_id = 18;
                        $query = "UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . $order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$invoice['order_id'] . "'";
                        $this->db->query($query);
                    }
    ...
    public function getInvoice($order_id)
    {
        ...
        $msg = "SELECT *, (SELECT CONCAT(c.firstname, ' ', c.lastname) FROM " . DB_PREFIX . "customer c WHERE c.customer_id = o.customer_id) AS customer FROM `" . DB_PREFIX . "order` o WHERE o.order_id = '" . (int)$order_id . "'";
        ...
        if ($order_query->num_rows) {
            ...
            return array(
                ...
                'payment_id' => $order_query->row['payment_id'],
                'payment_tran_id' => $order_query->row['payment_tran_id'],
                'payment_ref_id' => $order_query->row['payment_ref_id'],
    ...
    {
        //$sql = "SELECT i.invoice_id, i.order_id, i.order_product_id, i.invoice_no, i.shp_quantity, i.inv_quantity, i.price, i.total FROM `" . DB_PREFIX . "invoice` i";

        $sql = "
                SELECT i.invoice_id, i.order_id, i.order_product_id, i.invoice_no, i.shp_quantity, i.inv_quantity, i.price, i.total,o.date_added,os.name from `" . DB_PREFIX . "invoice` i" .
            " LEFT JOIN `order` o
                        ON o.invoice_no = i.invoice_no
        ...
        if (!empty($data['filter_invoice_no'])) {
            $sql .= " AND i . invoice_no = '" . $data['filter_invoice_no'] . "'";
        }

        if (!empty($data['filter_inv_quantity'])) {
            $sql .= " AND i . inv_quantity = '" . (int)$data['filter_inv_quantity'] . "'";
        }

        if (!empty($data['filter_price'])) {
            $sql .= " AND i . price = '" . (float)$data['filter_price'] . "'";
        }

        if (!empty($data['filter_total'])) {
            $sql .= " AND i . total = '" . (float)$data['filter_total'] . "'";
        }
    ...
    {
        $query = $this->db->query("SELECT order_id AS OrderId FROM " . DB_PREFIX . "order_product WHERE order_product_id = '" . (int)$order_product_id . "'");
    ...
    {
        $query = $this->db->query("SELECT DISTINCT order_id FROM `" . DB_PREFIX . "invoice` WHERE invoice_no = '" . $invoice_no . "'");
        return $query->rows[0]['order_id'];
    }
    ...
    public function getOrderProductTaxRate($order_product_id)
    {
    ...
    public function getOrderProductsByProductId($order_product_id)
    {
    ...
    public function getInvoiceProducts($invoice_no)
    {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "invoice WHERE invoice_no = '" . $invoice_no . "'");
    ...
    public function getTotalOrders($data = array())
    {
        ...
        if (!empty($data['filter_invoice_no'])) {
            $sql .= " AND invoice_no = '" . $data['filter_invoice_no'] . "'";
        }

        if (!empty($data['filter_inv_quantity'])) {
            $sql .= " AND inv_quantity = '" . (int)$data['filter_inv_quantity'] . "'";
        }
    ...
    public function getTotalQuantityByOrderId($order_id)
    {
        $txt_qry = "SELECT SUM(quantity) AS total_quantity FROM `" . DB_PREFIX . "order_product` WHERE order_id = '" . (int)$order_id . "'";
        $query = $this->db->query("SELECT SUM(quantity) AS total_quantity FROM `" . DB_PREFIX . "order_product` WHERE order_id = '" . (int)$order_id . "'");
    ...
    public function addOrderHistory($order_id, $data)
    {
        ...
        if ($data['notify']) {
            ...
            if ($data['shp_quantity']) {
                $message .= $language->get('text_shp_quantity') . "\n\n";
                $message .= strip_tags(html_entity_decode($data['shp_quantity'], ENT_QUOTES, 'UTF-8')) . "\n\n";
            }

            if ($data['inv_quantity']) {
                $message .= $language->get('text_inv_quantity') . "\n\n";
                $message .= strip_tags(html_entity_decode($data['inv_quantity'], ENT_QUOTES, 'UTF-8')) . "\n\n";
            }
    ...
    public function getOrderHistories($order_id, $start = 0, $limit = 10)
    {
        ...
        $query = $this->db->query("SELECT oh . date_added, os . name AS status, oh . comment, oh . notify, oh . shp_quantity FROM " . DB_PREFIX . "order_history oh LEFT JOIN " . DB_PREFIX . "order_status os ON oh . order_status_id = os . order_status_id WHERE oh . order_id = '" . (int)$order_id . "' AND os . language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY oh . date_added ASC LIMIT " . (int)$start . "," . (int)$limit);
