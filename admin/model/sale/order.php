...
class ModelSaleOrder extends Model
{
    ...
    {
        $nextInvoiceId = $this->getNextInvoiceId();
        if (isset($data['order_invoice'])) {
            foreach ($data['order_invoice'] as $order_invoice) {
                $order_id1 = $this->request->get['order_id'];
                $order_info = $this->model_sale_order->getOrder($order_id1);

                $this->log->write('\r\n**** invoice_no = ' . $order_info['invoice_no']);

                if ($order_info['invoice_no']) {
                    $invoice_no = $order_info['invoice_prefix'] . $order_info['invoice_no'] . "-" . $nextInvoiceId;
                } else {
                    $invoice_no = $this->createInvoiceNo($order_id1) . "-" . $nextInvoiceId;
                }

                $inv_total = (float)($order_invoice['inv_quantity'] * $order_invoice['price']);

                if ((int)$order_invoice['inv_quantity'] > 0) {
                    $this->db->query("INSERT INTO " . DB_PREFIX . "invoice SET order_id = '" . (int)$order_id1 . "', order_product_id = '" . (int)$order_invoice['order_product_id'] . "', invoice_no = '" . $invoice_no . "', inv_quantity = '" . (int)$order_invoice['inv_quantity'] . "', price = '" . (float)$order_invoice['price'] . "', total = '" . $inv_total . "'");
                }
            }

            /*foreach ($data['totals'] as $total) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "invoice_total SET invoice_id = '" . (int)$invoice_id . "', code = '" . $this->db->escape($total['code']) . "', title = '" . $this->db->escape($total['title']) . "', text = '" . $this->db->escape($total['text']) . "', `value` = '" . (float)$total['value'] . "', sort_order = '" . (int)$total['sort_order'] . "'");
            }*/
        }
    }
    ...
    public function editOrder($order_id, $data)
    {
        ...
        //Prabhu
        if (isset($this->request->post['order_invoices'])) {
            $order_products = $this->request->post['order_invoices'];
        ...
        $old_shp_quantity = array();
        //Prabhu
        //Store shp_quantity before deleting the order
        if (isset($data['order_product'])) {
            foreach ($data['order_product'] as $order_product) {
                //Prabhu
                $shp_quantity = $this->getShippedQuantity($order_id, $order_product['order_product_id']);
                if ($shp_quantity) {
        ...
        if (isset($data['order_product'])) {
            foreach ($data['order_product'] as $order_product) {
                //get the shp_quantity for rescpective order_product
                $shp_quantity1 = 0;
                foreach ($old_shp_quantity as $old_shp_qty) {
                    if ($old_shp_qty['order_product_id'] == $order_product['order_product_id']) {
                        $shp_quantity1 = $old_shp_qty['shp_quantity'];
                        break;
                    }
                }

                $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_product_id = '" . (int)$order_product['order_product_id'] . "', order_id = '" . (int)$order_id . "', product_id = '" . (int)$order_product['product_id'] . "', name = '" . $this->db->escape($order_product['name']) . "', model = '" . $this->db->escape($order_product['model']) . "', quantity = '" . (int)$order_product['quantity'] . "', price = '" . (float)$order_product['price'] . "', total = '" . (float)$order_product['total'] . "', shp_quantity = '" . (int)$shp_quantity1 . "', tax = '" . (float)$order_product['tax'] . "', reward = '" . (int)$order_product['reward'] . "'");
    ...
    public function completeOrder($order_id)
    {
        ...
        $this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = 5 WHERE order_id = '" . (int)$order_id . "'");
    }

    public function updateOrderStatusDup($order_id, $status)
    {
    ...
    public function getOrder($order_id)
    {
        ...
        if ($order_query->num_rows) {
            ...
            return array(
                ...
                'order_weight' => $order_query->row['order_weight'],
                'affiliate_id' => $order_query->row['affiliate_id'],
                ...
                'inv_quantity' => $order_product_query->row['inv_quantity']
            );
        } else {
            return false;
        }
    ...
    function updateWeight($order_id, $order_weight)
    {
        $sql = "UPDATE `" . DB_PREFIX . "order` SET order_weight = '" . $order_weight . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'";
    ...
    //Prabhu
    public function getShippedQuantity($order_id, $order_product_id)
    {
        $sql = "SELECT shp_quantity FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'" . " AND order_product_id = '" . (int)$order_product_id . "'";
        $query = $this->db->query($sql);
        return $query->row['shp_quantity'];
    }

    public function getShippedQuantityByOrderId($order_id)
    {
        $sql = "SELECT shp_quantity FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'";
        $query = $this->db->query($sql);
        return $query->row['shp_quantity'];
    }

    //Prabhu - Start
    public function updateShipQuantity($order_id, $order_invoice, $shp_quantity, $order_status_id)
    {
        $order_product_id = $order_invoice['order_product_id'];
        $this->db->query("UPDATE `" . DB_PREFIX . "order_product` SET shp_quantity = '" . (int)$shp_quantity . "' WHERE order_product_id = '" . (int)$order_product_id . "'");
    }

    public function updateOrderStatus($order_id, $order_invoice, $shp_quantity, $order_status_id)
    {
        $order_product_id = $order_invoice['order_product_id'];
        $order_count = $this->getTotalOrdersByOrderId($order_id);
        $shp_count = $this->getTotalShippedCountByOrderId($order_id);

        if (($shp_quantity == $order_count)) {
            $order_status_id = 3;
            $query = "UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . $order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'";
            $this->db->query($query);
        } //elseif(($shp_quantity < $order_invoice['quantity']) && ($order_invoice['inv_quantity'] > 0)){
        elseif (($shp_count < $order_count) && $shp_count > 0) {
            $order_status_id = 18;
            $query = "UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . $order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'";
            $this->db->query($query);
        }
    ...
    public function getTotalOrdersByOrderId($order_id)
    {
    ...
    public function getTotalShippedCountByOrderId($order_id)
    {
        $query = $this->db->query("SELECT SUM(shp_quantity) AS total FROM `" . DB_PREFIX . "order_product` WHERE order_id = '" . (int)$order_id . "'");
    ...
    {
        //get last id
        $result = $this->db->query('SELECT MAX(invoice_id) FROM ' . DB_PREFIX . 'invoice');
        $id = $result->rows[0]['MAX(invoice_id)'] + 1;
